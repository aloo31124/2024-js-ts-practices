...
function createArray1(length: number, defaul: any): Array<any> {
    let result = [];
...
function createArray2<T>(length: number, defaul: T): Array<T> {
    let result: T[] = [];
    for(let i=0; i<length; i++) {
        result[i] = defaul; //報錯: Type 'any' is not assignable to type 'never'.ts
    }
...
console.log(createArray2<string>(5, "yo2-1"));
...
function loggingArray<T extends Lengthwise>(arr: T): T {
